-- Recoloca progression_rules na RPC atômica de persistência de planos.
--
-- A coluna foi criada e gravada pela migration 0010, mas as migrations 0013 e
-- 0014 recriaram save_training_plan com listas explícitas de colunas sem
-- progression_rules. Como o PostgREST continuava aceitando o JSON completo, a
-- regressão descartava as regras em silêncio. Esta migration parte da versão
-- integral da RPC da 0014 e altera somente o INSERT de training_plans.
...
-- Asserções cumulativas: a 0015 deve falhar cedo se a coluna ou qualquer campo
-- de cardio preservado da 0014 deixar de existir ou de ser gravado pelas RPCs.
do $$
declare
  v_col text;
begin
  if not exists (
    select 1 from information_schema.columns
     where table_schema = 'public' and table_name = 'training_plans'
       and column_name = 'progression_rules'
  ) then
    raise exception 'asserção falhou: training_plans.progression_rules não existe';
  end if;
  ...
  foreach v_col in array array['metric', 'target_duration_seconds', 'target_distance_m', 'progression_rules'] loop
    if position(v_col in pg_get_functiondef(
