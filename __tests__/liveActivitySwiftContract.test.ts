...
// Ordem canônica dos campos do ContentState — D-10 manda neonColor ao fim,
// para que payloads legados (sem o campo) continuem decodificáveis e o
// memberwise initializer trate o opcional com default nil.
...
describe('paridade dos dois SessionActivityAttributes.swift', () => {
  ...
  it('declaram os campos na ordem canônica, com neonColor String? opcional ao fim', () => {
    for (const [rotulo, src] of [
      ...
      expect(campos.map((c) => c.nome)).toEqual([...ORDEM_ESPERADA]);
...
describe('ponte Expo (LiveActivityModule.swift)', () => {
  it('Record espelha a mesma ordem com @Field opcional default nil', () => {
  ...
  it('contentState(from:) encaminha neonColor como último argumento', () => {
    ...
    expect(args).toEqual([...ORDEM_ESPERADA]);
...
describe('layout, tipografia e demais elementos preservados', () => {
  ...
  it('mantém timers, intents e decoração da Activity', () => {
    ...
    expect(widgetSwift).toContain('widgetURL(URL(string: "forcaapp://session/active"))');
    expect(widgetSwift).toMatch(
