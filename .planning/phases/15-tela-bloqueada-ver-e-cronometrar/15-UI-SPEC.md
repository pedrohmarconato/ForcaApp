...
| Compact (Dynamic Island compact) | `.caption` (≈12pt) with `.monospacedDigit()` | `.semibold` (600) | `compactLeading`/`compactTrailing` text (mm:ss or "3/4") — see D-02 and Open Decision on `minimal` below |
...
Weights used: **`.bold` and `.regular`**, plus `.semibold` reserved strictly for the space-constrained `compactLeading`/`compactTrailing` digits (legibility at ~12pt in a pill 44pt wide needs more visual weight than regular; this is the native-platform equivalent of the "big number gets more weight" rule already used by `SessionPlayer.tsx`'s rest clock). Do not introduce a fourth weight.
...
| overflow | `compactLeading`/`compactTrailing` (Dynamic Island compact) | ✅ covered | Fixed to numeric-only content (`mm:ss` or `N/N`) by D-02 itself — no exercise name, no free text ever enters the ~44pt-wide compact regions, so overflow is structurally prevented rather than truncated |
